...
public final class Point {
	...
	public boolean inRange(Point pLU, Point pRD) {
		return ((pRow >= pLU.row()) && (pRow <= pRD.row()) && (pCol >= pLU.col()) && (pCol <= pRD.col()));
	}

	/**	此点是否在世界范围内	*/
	public boolean inWorld() {
		return inRange(new Point(0, 0), new Point(Global.rowNum - 1, Global.colNum - 1));
	}
	...
	/**	得到距离p点的距离（行差或列差最大的一个）	*/
	public int distance(Point p) {
		return Integer.max(Math.abs(pRow - p.pRow), Math.abs(pCol - p.pCol));
	}
	...
	/**	向右下方向移动此点，相对位移(d.row(), d.col())	*/
	public Point mov(int r, int c) {
		return new Point(pRow + r, pCol + c);
	}
	
	/**	该点表示方向，则可以乘以行列距离	*/
	public void multi(int dr, int dc) {
		pRow *= dr;
		pCol *= dc;
	}
